#!/usr/bin/env python3
"""
Fire Claim EMLX Duplicate Audit — UPDATED

Purpose:
    Find likely duplicate .emlx emails using several signals:
      - Message-ID
      - raw content hash
      - normalized subject + people + body hash
      - file size + date bucket + from + to + subject
      - same sender + same day + same normalized body
      - fuzzy comparison for near-duplicates

This updated version is designed to catch Apple Mail draft/outbox/sent repeats,
where the same sent email appears many times with slightly different timestamps.

It does NOT delete, move, or alter any original email files.

Recommended run from Git repo root:

    python3 scripts/audit_emlx_duplicates.py FireClaimEmail_Claude duplicate_audit_round2

Then quarantine duplicates with:

    python3 scripts/safe_remove_duplicate_emlx.py --audit-dir duplicate_audit_round2 --quarantine-dir duplicate_quarantine_round2 --apply

Outputs:
    duplicate_audit_round2/duplicate_groups.csv
    duplicate_audit_round2/all_email_fingerprints.csv
    duplicate_audit_round2/README.md
"""
...
DEFAULT_DATE_WINDOW_SECONDS = 180
...
class EmailFP:
    ...
    duplicate_key_sender_day_body: str
...
def parse_date(msg: Optional[Message], filename: str, window_seconds: int) -> tuple[Optional[datetime], str, str, str]:
    ...
    if dt is None:
        m = re.match(r"(\d{4}-\d{2}-\d{2})[_-](\d{2})[-_](\d{2})[-_](\d{2})", filename)
        if m:
            try:
    ...
    if dt is None:
        return None, "", "unknown_day", "unknown_bucket"
    ...
    return dt, dt.isoformat(), day, bucket
...
def normalize_body(body: str) -> str:
    ...
    # Remove obvious quoted reply chains/signatures. We want the actual composed content.
    body = re.sub(r"(?is)\bOn .{0,250}? wrote:\s*.*$", "", body)
    body = re.sub(r"(?is)From:\s+.*?Subject:\s+.*$", "", body)
    body = re.sub(r"(?is)Begin forwarded message:.*$", "", body)
    body = re.sub(r"(?is)Sent from my iPhone.*$", "", body)

    # Remove volatile link wrapping / tracking-ish noise.
...
def fingerprint(path: Path, window_seconds: int) -> EmailFP:
    ...
    dt, date_iso, date_day, date_bucket = parse_date(msg, path.name, window_seconds)
    ...
    body_hash = sha256_text(normalized_body[:50000])
    ...
    # Important new rule:
    # Apple Mail can leave repeated sent/draft/outbox copies with different timestamps.
    # This catches same body from same sender on same day, even if filenames/times differ.
    duplicate_key_sender_day_body = "|".join([from_email, date_day, body_hash])
    ...
    return EmailFP(
        ...
        raw_hash=raw_hash,
        body_hash=body_hash,
        body_preview=normalized_body[:1000],
        duplicate_key_strict=duplicate_key_strict,
        duplicate_key_loose=duplicate_key_loose,
        duplicate_key_sender_day_body=duplicate_key_sender_day_body,
    )
...
def canonical_for_group(group: list[EmailFP]) -> EmailFP:
    """
    Suggested keep rule:
        - keep latest timestamp, not earliest, for draft/send duplicates.
        - if no dates, keep shortest filename as stable fallback.
    """
    return sorted(
        group,
        key=lambda x: (
...
def fuzzy_groups(items: list[EmailFP], threshold: float) -> list[list[EmailFP]]:
    ...
    for item in items:
        key = "|".join([str(item.size), item.subject_key, item.from_email])
    ...
    for group in pre_groups.values():
        ...
        for i, a in enumerate(group):
            ...
            for j, b in enumerate(group[i + 1 :], start=i + 1):
                if j in used:
                    continue
                if a.date and b.date and abs((a.date - b.date).total_seconds()) > 24 * 60 * 60:
                    continue
...
def add_group_rows(rows: list[dict], group_type: str, groups: list[list[EmailFP]]) -> None:
    for group_no, group in enumerate(groups, start=1):
        ...
        for item in sorted(group, key=lambda x: (x.date or datetime.min.replace(tzinfo=timezone.utc), x.filename)):
            rows.append(
                {
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Audit .emlx duplicates by size, date, sender/recipient, subject, and content.")
        ...
        all_rows.append(
            {
                ...
                "sender_day_body_key": fp.duplicate_key_sender_day_body,
            }
        )
    ...
    strict_groups = list(group_by(fps, "duplicate_key_strict").values())
    loose_groups = list(group_by(fps, "duplicate_key_loose").values())
    sender_day_body_groups = list(group_by(fps, "duplicate_key_sender_day_body").values())
    ...
    add_group_rows(duplicate_rows, "same_subject_people_body", strict_groups)
    add_group_rows(duplicate_rows, "same_size_datebucket_from_to_subject", loose_groups)
    add_group_rows(duplicate_rows, "same_sender_same_day_same_body", sender_day_body_groups)
    add_group_rows(duplicate_rows, "fuzzy_same_size_sender_subject", fuzzy)

    write_csv(
        output_dir / "all_email_fingerprints.csv",
        all_rows,
        [
            "file",
            "date",
    ...
    readme = f"""# EMLX Duplicate Audit — Updated
